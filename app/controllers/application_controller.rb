class ApplicationController < ActionController::Base
  ...
  def beverage
    @jsonbevs = {name: 'bevs',children: beverage_counts = WordType.joins(:tweets).where(text: 'beverages').where('tweets.created_at > ?',Time.now - 1.hour).group('words.text').count.map{|k,v| [{'name'=> k,'size'=> v}]}.flatten}.to_json.html_safe
    respond_to do |f|
